...
**A claim-only match is not a re-raise.** Same invariant at a different anchor gets a new `F#` with a `Related:` note (`REVIEW_CONTRACT.md` §5). Suppression requires an anchor match. A finding at a symbol the old ruling never covered is new work, whatever it rhymes with, so it counts as a new `F#` in the `Progress:` line and in the loop-control rules below.

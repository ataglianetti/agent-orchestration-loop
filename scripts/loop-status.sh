...
is_frozen() {  # 0 = immutable flag is set on the hook config
...
[ -e "$SETTINGS" ] && is_frozen "$SETTINGS" && frozen=1
...
if [ "$soft_present" -eq 1 ] || [ "$hard_present" -eq 1 ]; then
  ...
  [ "$frozen" -eq 1 ]       && echo "  hook config frozen: $SETTINGS (unlock lifts it)"
  exit 0
fi

echo "INACTIVE"
echo "  no run marker for this repo ($REPO)"
# A frozen config with no marker is a stuck state — a lock whose unlock never ran.
[ "$frozen" -eq 1 ] && echo "  WARNING: $SETTINGS is still frozen — run: sudo ./.claude/scripts/unlock-loop.sh"
exit 1

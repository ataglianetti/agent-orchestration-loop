...
expect_block "agent runs unlock"       'sudo ./.claude/scripts/unlock-loop.sh'
expect_block "agent runs ungate"       'ungate'
expect_block "agent runs ungate --all" 'cd /repo && ungate --all'
expect_block "agent sources then ungates" 'source .claude/scripts/loop-gate.zsh; ungate'
expect_block "agent runs unlock by path" 'sudo -n /repo/.claude/scripts/unlock-loop.sh'
...
expect_pass "word containing ungate"   'grep -rn ungated_count src/'

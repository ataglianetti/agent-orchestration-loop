...
| ID  | Severity         | Where                                                        | Problem | Recommended fix |
| --- | ---------------- | ------------------------------------------------------------ | ------- | --------------- |
| G4  | CRITICAL-ON-FLIP | `scripts/lock-loop.sh` — `freeze_tree`                       | The hard lock freezes all of `.claude/`, including `.claude/worktrees/`, so any parallel worktrees become immutable while the lock holds. | Freeze only the guard surface: `settings*.json`, `scripts/`, `commands/`, `agents/`. Exclude `worktrees/`. |
